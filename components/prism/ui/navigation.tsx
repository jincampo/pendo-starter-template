...
const NavigationItem = React.forwardRef<HTMLDivElement, NavigationItemProps>(
  ({ icon, label, itemKey, selectedItem, isOpen = true, to }, ref) => {
    ...
    const content = (
      <div
        ref={ref}
        className={cn(
          "flex items-center box-content h-[26px] px-[6px] py-[6px] cursor-pointer transition-colors rounded-[5px] overflow-hidden",
          isSelected ? "bg-transparent" : "hover:bg-[var(--color-gray-110)]"
        )}
      >
        <div className={cn(
...
const Navigation = React.forwardRef<HTMLElement, NavigationProps>(
  ({ isOpen = true, selectedItem = 'product', onCollapseClick }, ref) => {
    return (
      <nav
        ref={ref}
        className={cn(
          "fixed left-0 top-0 h-full bg-[var(--color-gray-110)] flex flex-col p-4 transition-[width]",
          isOpen ? "w-[244px]" : "w-16"
        )}
      >
        <div className={cn(
        ...
        <div className="flex-1 flex flex-col gap-1">
          <NavigationItem icon={<Icon name="Home" size="small" />} label="Home" itemKey="home" selectedItem={selectedItem} isOpen={isOpen} />
          <NavigationItem icon={<Icon name="LayoutDashboard" size="small" />} label="Dashboards" itemKey="dashboards" selectedItem={selectedItem} isOpen={isOpen} />
          <NavigationItem icon={<Icon name="Package2" size="small" />} label="Product" itemKey="product" selectedItem={selectedItem} isOpen={isOpen} />
          <NavigationItem icon={<Icon name="Users" size="small" />} label="People" itemKey="people" selectedItem={selectedItem} isOpen={isOpen} />
          <NavigationItem icon={<Icon name="BarChart2" size="small" />} label="Analytics" itemKey="analytics" selectedItem={selectedItem} isOpen={isOpen} to="/chat" />
          <NavigationItem icon={<Icon name="Heart" size="small" />} label="NPS" itemKey="nps" selectedItem={selectedItem} isOpen={isOpen} />
          <NavigationItem icon={<Icon name="MessageSquare" size="small" />} label="Guides" itemKey="guides" selectedItem={selectedItem} isOpen={isOpen} />
          <NavigationItem icon={<Icon name="PlaySquare" size="small" />} label="Orchestrate" itemKey="orchestrate" selectedItem={selectedItem} isOpen={isOpen} />
          <NavigationItem icon={<Icon name="Headphones" size="small" />} label="Listen" itemKey="listen" selectedItem={selectedItem} isOpen={isOpen} />
        </div>
        
        <div className="mt-auto flex flex-col gap-1">
          <NavigationItem icon={<Icon name="Bell" size="small" />} label="Notifications" itemKey="notifications" selectedItem={selectedItem} isOpen={isOpen} />
          <NavigationItem icon={<Icon name="Settings" size="small" />} label="Settings" itemKey="settings" selectedItem={selectedItem} isOpen={isOpen} to="/settings" />
        </div>
